...
function canDeleteRoute($conn, $route_id, $user_id) {
    ...
    if ($result->num_rows > 0) {
        ...
        return $route['created_by'] == $user_id;
    }
  ...
  .sidebar {
    ...
    height: auto;
        ...
        *:focus {
        ...
        /* User location marker should be below routes */
        .user-location-marker {
...
<body>
  ...
  <i class="fas fa-list"></i>
    ...
    <div class="modal" id="deleteRouteModal">
        <div class="modal-content p-3">
            <div class="modal-header">
                <h4 class="mb-0">Konfirmasi Penghapusan</h4>
                <button class="close-btn" data-dismiss="delete-route">&times;</button>
            </div>
            <div class="modal-body">
                <p>Apakah Anda yakin ingin menghapus rute ini? Semua laporan yang terkait dengan rute ini juga akan dihapus. Tindakan ini tidak dapat dibatalkan.</p>
            </div>
            <div class="modal-footer" style="padding: 1rem; display: flex; gap: 0.5rem; justify-content: flex-end;">
                <button class="btn btn-secondary" id="cancelDeleteRoute">Batal</button>
                <button class="btn btn-danger" id="confirmDeleteRoute">Hapus</button>
            </div>
        </div>
    </div>
    ...
    `;
            ...
            // Also call it when the map view changes
...
function selectSearchResult(index) {
    ...
    searchLocation(result.display_name);
            ...
            function checkRouteOwnership(routeId) {
                fetch(`check_route_ownership.php?route_id=${routeId}`)
                .then(response => response.json())
                .then(data => {
                ...
                .catch(error => {
                    console.error('Error checking route ownership:', error);
                    document.getElementById('routeActions').style.display = 'none';
                });
            }
            ...
            document.getElementById('deleteRouteBtn').addEventListener('click', function() {
                if (selectedRoute) {
                    showDeleteRouteModal(selectedRoute.id);
                }
            });
            ...
            function showDeleteRouteModal(routeId) {
                ...
                document.getElementById('cancelDeleteRoute').onclick = function() {
                    modal.style.display = 'none';
                };
                
                document.getElementById('confirmDeleteRoute').onclick = function() {
                    deleteRoute(routeId);
                };
            }
            
            // Close delete modal when clicking the close button
            document.querySelector('[data-dismiss="delete-route"]').addEventListener('click', function() {
            ...
            function deleteRoute(routeId) {
                ...
                .catch(error => {
                    console.error('Error:', error);
                    alert('Terjadi kesalahan saat menghapus rute');
                });
            }
